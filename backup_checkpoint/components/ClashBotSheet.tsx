...
export default function ClashBotSheet({
  ...
  // Default DARK to match your app scheme
  ...
  console.log("KB HEIGHT:", h);
  setKbHeight(h);
});  
    const s2 = Keyboard.addListener(hideEvt as any, () => setKbHeight(0));
  ...
  // 🔥 The real fix: dynamically size the sheet when keyboard appears
  const screenH = Dimensions.get("window").height;

  // Base visible height of sheet when keyboard is hidden
  const baseSheetHeight = Math.floor(screenH * 0.92);

  // When keyboard shows, reduce the sheet height so it never sits behind the keyboard
  // Extra 18px gives breathing room above the keyboard
  const computedSheetHeight =
    kbHeight > 0 ? Math.max(320, screenH - kbHeight - 18) : baseSheetHeight;

  // Also pad the ScrollView bottom so last items can scroll above keyboard
  const scrollBottomPad = kbHeight > 0 ? kbHeight + 24 : 24;
  ...
  return (
    ...
    >
      ...
      >
        {/* Brand glow strip */}
        <LinearGradient
          colors={["rgba(36,230,184,0.26)", "rgba(38,198,255,0.16)", "rgba(0,0,0,0)"]}
          start={{ x: 0, y: 0 }}
          end={{ x: 1, y: 0 }}
        ...
        <View style={styles.header}>
          ...
          <Pressable
            style={[styles.closeBtn, { backgroundColor: theme.btnBg, borderColor: theme.btnBorder }]}
            onPress={onClose}
          >
        ...
        <ScrollView
          style={{ flex: 1 }}
          contentContainerStyle={[styles.scrollGrow, { paddingBottom: scrollBottomPad }]}
